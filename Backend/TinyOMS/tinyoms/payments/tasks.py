...
def prepare_client_message(installment, time_zone):
    ...
    template_str = conf.value if conf else \
        "You have a debt of $currency$amount with a deadline of $pay_before. You can omit this message if you already paid."
    installment_reminder_template = Template(template_str)
    pay_before = installment.pay_before.astimezone(time_zone)
    currency = installment.currency
    amount = installment.amount
    message_prefix = Configuration.objects.filter(key="installment_reminder_prefix").first()
    ...
    message = message_prefix + installment_reminder_template.substitute(
        pay_before=pay_before.strftime("%Y-%m-%d"),
        currency=currency,
        amount=amount
    )
    return message
...
def installment_reminder_task(email, subject, title, message):
    content = loader.render_to_string(
        "email-installment-reminder.html", 
        {"title": title, "message": message}
    )
    mail = EmailMessage(
        subject, content, to=[email], from_email=EMAIL_FROM_ADDRESS
    )
...
def check_late_payments(year, month, day):
    ...
    values_fields = ("payer__first_name", "payer__last_name", "currency", "amount", "pay_before")
    ...
    older_late_payments = Installment.objects.filter(
        paid_at__isnull=True,
        pay_before__lt=task_date_time + timedelta(days=-1)
    ).values(*values_fields)
...
def create_late_payments_task(today, time_zone):
    ...
    if not (Task.objects.filter(verbose_name=verbose).exists() or CompletedTask.objects.filter(verbose_name=verbose).exists()):
        check_late_payments(
